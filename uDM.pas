...
begin
  len:=socket.ReceiveLength;
   {
  if(len=sizeof(stPackageHeader))then
  begin
    socket.ReceiveBuf(mHeader,len);
    bHeader:=true;
    exit;
  end;
  }
  ...
  SaveSocketDataToFile(socket.SocketHandle,0,len,p);
  //uHookData.SaveFile('clientread(8300)',p^,len);
  if(len=sizeof(stPackageHeader))then
  begin
    copymemory(@mheader,p,sizeof(stPackageHeader));
    if(not VerifyPackageHeader(@mheader))then exit;
    bHeader:=true;
    exit;
  end;
  if(bHeader)then
  begin
    header:=mHeader;
    setLength(data,mheader.dwDataSize);
    copymemory(@data[1],p,mheader.dwDataSize);
    Log('ss1ClientRead:'+data);
    decryptdata:=flash.decryptData(data);
    Log('ss1ClientRead:'+decryptdata);
    fmain.memoInfo.Lines.Add('---------------------------------------');
